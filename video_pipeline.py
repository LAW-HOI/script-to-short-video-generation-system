...
from services.subtitle_utils import (
    SrtBlock,
...
class SubtitleComposer:
    ...
    def generate_srt_from_whisper(
        ...
        run_command(cmd)
        self.reflow_srt(output_path, max_chars=max_chars, language=language)
        if subtitle_offset:
            self.apply_srt_offset(output_path, subtitle_offset)
        return output_path

    def reflow_srt(self, subtitle_path: Path, max_chars: int, language: str | None = None) -> Path:
        if not subtitle_path.exists():
            return subtitle_path
        blocks = parse_srt_blocks(subtitle_path.read_text(encoding="utf-8"))
        if not blocks:
            return subtitle_path
        rebuilt: list[SrtBlock] = []
        for block in blocks:
            resolved_language = language or detect_text_language(block.text)
            pieces = split_script_text(block.text, max_chars, language=resolved_language)
            if len(pieces) <= 1:
                rebuilt.append(block)
                continue
            weights = [subtitle_weight(piece, resolved_language) for piece in pieces]
            total_weight = max(sum(weights), 1)
            current = block.start
            duration = max(block.end - block.start, 0.1)
            for piece, weight in zip(pieces, weights):
                share = duration * (weight / total_weight)
                start = current
                current += share
                end = max(start + 0.1, current)
                rebuilt.append(SrtBlock(index=0, start=start, end=end, text=piece.strip()))
        subtitle_path.write_text(render_srt_blocks(rebuilt), encoding="utf-8")
        return subtitle_path
